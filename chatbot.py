...
plot_description = """In the heart of the bustling metropolis of Astoria, the old Henderson House stands as a
 ...
 Whenseventeen-year-old Mia Alvarez moves to Astoria with her family, she is immediately
...
<{plot_description}>
Write this as one paragraph and make the summarization exciting. Write from the point of view of someone from the Henderson family. Also include the title of the book in your summary.
"""
...
book_summary = get_response(model, messages)
...
book_reviews ={
    ...
    "I struggled to connect with the characters in this book. They felt flat and underdeveloped, which made it difficult to care about their fates. The plot, while intriguing, was ultimately let downbythe lack of depth in the characters.",
...
for review in book_reviews:
    review_prompt = f"""
    Determine the sentiment of the following book review as a single word, positive or negative.
    {review}
    """
    ...
    sentiment = get_response(model, review_messages)
...
book_review_positive = ''

for review in book_reviews_with_sentiments:
    if review['sentiment] == "positive":
        book_review_positive.add(review['review'] + " ")

email_prompt = f"""
Generate an email for my book release announcement email using {book_summary} and {book_review_positive}. Make sure to create an email subject line. Make the email exciting!
"""
...
email = get_response(model, email_message)
